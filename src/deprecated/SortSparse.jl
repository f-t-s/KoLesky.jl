...
# Function that constructs the ordering and sparsity pattern from an input Vector of Points
function sortSparseRev(x::AbstractVector{<:Point}, ρ, squared_distance=squared_euclidean_distance, initInd::Int=1)
  dist2Func(i::Int, j::Int) = squared_distance(x[i], x[j])
  N = length(x)
  return sortSparseRev(N, dist2Func, ρ, initInd)
end
...
function ordering_and_skeletons(N, ρ, λ, dist2Func, initInd=1)
  colptr, rowval, P, revP, distances = sortSparseRev(N, dist2Func, ρ, initInd)
  skeletons = construct_skeletons(colptr, rowval, distances, λ)
  return P, revP, distances, skeletons
end

function ordering_and_skeletons(x::AbstractVector{<:Point}, ρ, λ, squared_distance=squared_euclidean_distance, initInd::Int=1)
  dist2Func(i::Int, j::Int) = squared_distance(x[i], x[j])
  N = length(x)
  return ordering_and_skeletons(N, ρ, λ, dist2Func, initInd)
end
